...
_nInputs(nInputs), _nOut(nOuts), _fitness(0), _recurrent(recurrent)
{
	...
	{
		//Liga cada entrada � cada saida por meio um gene sinaptico
		//Imaginariamente existem os neuronios de entrada cada um com seu id(mas s� uma representacao deles)
		//Cria o node
...
void Genome::AddConnection()
{
	...
	//A distribuicao de entrada s� poderao ser n�s
	std::uniform_int_distribution<size_t> secondNodeIdDist(0, _nodes.size() - 1);
	//Usado para saber se ser� recorrent
	...
	do
	{
		...
		//Id real do node
		size_t secondNodeId = _nodes[secondId]._id; //N�o leva em conta as entradas
		//Conex�o recorrente?
		bool recurrent;
		//Se o primeiro node esta na mesma camada ou em uma camada mais atr�s eh obrigat�riamente recorrente
		...
		//Verifica em cada gene do node se ele ja n�o tinha essa conex�o
...
void Genome::AddNode()
{
	...
	//Garante que esse novo n� j� n�o existe
	while (true)
	{
		//Itaretor do gene que ser� afetado
		auto gene = _genes.begin();
		//Desloca o gene para um gene aleat�rio
		...
		double y = (y1 + y2) / 2;
		bool exist = false;
		//Verifica se ja existe um n� nessa posicao
...
{
	//Novo Genoma <- o genoma que chamu a fun��o (melhor genoma)
		...
		for (size_t nMutation = 0; nMutation < nMutations; nMutation++)
		{
			...
			if (prob(_randomEng))
			{
				...
				if (prob(_randomEng)) {
					...
					size_t count = 0;
					bool mutate(false);
					//Probabilidade de sofrer mutacao nos pesos seguindo a distribui��o gauseana
					prob = std::uniform_int_distribution<int>(0, 90);
					do
					{
